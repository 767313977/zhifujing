  ...
  const toolbar = (
    <>
      ...
      <button
        ...
        className="num border border-accent/60 bg-accent/10 px-2.5 py-[3px] text-[12px] text-accent transition-colors hover:bg-accent/20 disabled:cursor-not-allowed disabled:opacity-40"
      >
        {syncing ? '同步中…' : '同步行情'}
      </button>
    </>
    ...
    <Layout toolbar={toolbar}>
      ...
      <div className="space-y-4">
        ...
        >
          <div className="flex flex-col gap-2 px-4 py-3.5 sm:flex-row">
            <input
              ...
              className="num min-w-0 flex-1 border border-line bg-ink-850 px-3 py-2 text-[13px] text-fg outline-none placeholder:text-fg-dim focus:border-fg-dim"
            />
            <button
              type="button"
              onClick={() => void add()}
              className="shrink-0 border border-line px-5 py-2 text-[13px] text-fg-muted transition-colors hover:border-fg-dim hover:text-fg"
            >
              加入
            </button>
          </div>
        ...
        <Panel
          ...
          delay={120}
        >
          <div className="px-4 py-3 text-[12px] leading-relaxed text-fg-dim">

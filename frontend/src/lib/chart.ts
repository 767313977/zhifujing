...
/**
 * 同花顺（THS）K 线配色。**这些值是从用户截图上逐像素采出来的，不是照印象调的** ——
 * 一张 2469×692 的同花顺截图里：
 *
 * | 元素 | 值 | 采样依据 |
 * | --- | --- | --- |
 * | 背景 | `#10161a` | 占 91.8% 像素 |
 * | 网格 | `#31393f` | x 覆盖 2466/2469 列（横贯的线） |
 * | 阳线 | `#d90000` | 像素集中在主图带 y26~425 |
 * | 阴线 | `#17def1` | 同上 |
 * | 成交量柱 涨/跌 | `#fe3330` / `#56fbfc` | 像素集中在副图带 y440~692 |
 * | 均量线 | `#ffff00` | x 覆盖 2165 列 |
 *
 * ⚠️ 只给 `KLineChart` 用。**不要拿它替换 `CHART.up/down`** —— 表格、板块热力、
 * 情绪周期那些地方仍是站点自己那套砖红/松绿，把红涨青跌扩散到全站是另一个决定。
 *
 * ⚠️ 这套配色**本身亮度不对等**（阴线 `#17def1` 的 WCAG 相对亮度 0.588，阳线
 * `#d90000` 只有 0.148，差 4 倍；本站自己那套是刻意对齐过的）。同花顺就是这么画的，
 * 照着抄属于「要这个味」，但要知道代价：阴线会明显比阳线跳 —— 改这两个值时先算一遍。
 */
export const THS = {
  bg: '#10161a',
  grid: '#31393f',
  ...
  volUp: '#fe3330',
  volDown: '#56fbfc',
  /** 副图均量线。截图里能确认的只有那条黄的（MAVOL5），MAVOL10 用品红是同花顺默认 */
  volMa5: '#ffff00',
  volMa10: '#ff00ff',
} as const

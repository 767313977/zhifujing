 ...
 *
 *  2026-09-26 全站改同花顺配色，这里整份跟着换。**采样依据与对比度、以及那对
 *  红/青亮度不对等的代价，都写在 `index.css` 的令牌注释里**，不要在本地另立一套解释。
 */
export const CHART = {
  // 涨跌：同花顺的亮红 / 青（截图采样，理由见 index.css 的涨跌色注释）。
  // 跌色是从采样值 #56fbfc 两次压下来的（→ #3ec9cc → #45bfc4）：与红的对比度落差
  // 从 2.9 倍收到 1.66 倍，7.71:1 仍远高于 AA。改动只认 index.css 那一处说明。
  up: '#fe3330',
  down: '#45bfc4',
  accent: '#e0b055',
  fg: '#d4dce2',

...
function AmountTrend({
  ...
  if (points.length === 0 || values.length === 0) {
    return <span className="num text-[13px] text-fg-dim">沪指 + 深成指（无历史序列）</span>
  }
  ...
  return (
    <div>
      ...
      <div className="flex gap-1.5 pt-1.5">
        <div className="relative shrink-0" style={{ width: AXIS_WIDTH, height: BAR_AREA }}>
          {ticks.map((value) => (
            <span
              key={value}
              className="num absolute right-0 text-[12px] leading-none text-fg-dim"
      ...
      <div className="num mt-1.5 flex flex-wrap items-center gap-x-2.5 gap-y-0.5 text-[13px] text-fg-dim">
        <span>
    ...
    >
      <div className="grid grid-cols-2 overflow-hidden md:grid-cols-4">
        <Metric
          ...
          tone="text-up"
          sub={<span className="num text-[13px] text-fg-dim">含首板与连板</span>}
        />
        <Metric
          label="跌停"
          value={fmtInt(s.limit_down_count)}
          tone="text-down"
          sub={<span className="num text-[13px] text-fg-dim">当日跌停家数</span>}
        />
        <Metric
          label="炸板"
          value={fmtInt(s.broken_count)}
          tone="text-fg-muted"
          sub={<span className="num text-[13px] text-fg-dim">触板未封住</span>}
        />
        <Metric
          label="最高连板"
          value={fmtInt(s.max_consecutive)}
          tone="text-accent"
          sub={<span className="num text-[13px] text-fg-dim">市场高度</span>}
        />
            ...
            s.up_count != null && s.down_count != null ? (
              <div className="space-y-1.5">
                <Meter
                  ...
                  to="var(--color-down)"
                />
                <div className="num flex justify-between text-[13px] text-fg-dim">
                  <span className="text-up">涨 {s.up_count}</span>
                  <span className="text-down">跌 {s.down_count}</span>
                </div>
              </div>
            ) : (
              <span className="num text-[13px] text-fg-dim">历史数据源仅提供当日值</span>
            )
          }
        />
        <Metric

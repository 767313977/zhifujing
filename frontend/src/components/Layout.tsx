        ...
        */}
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-x-3 px-3 md:flex-nowrap md:gap-x-6 md:px-5">
          {/* 品牌：中文名做主标识，等宽拉丁字母做辅助标记。
              字号与字距都是响应式的：牌子从两个字变成七个字后，按原参数会从
              导航那里抢走约 80px 可视宽度（窄屏下导航本来就靠横滑，再挤就只剩
              百来 px）。窄屏用 13px/0.05em 收回来，宽屏才展开成 15px/0.1em。

              ⚠️ 拉丁标记的断点是 **xl（1280）不是 lg（1024）**：它占 94.6px，而
              1024~1279 正好是导航最紧张的区间（那一段导航本来就显示不全）。
              按 1024 显示的话，等于用「一个纯装饰」换掉整整一个导航项 ——
              实测 1024px 下导航从 6/9 掉到 5/9。1280 以上富余够，随便显示。 */}
          <div className="flex h-[52px] shrink-0 items-baseline gap-2">
            ...
            <span className="num hidden text-[10px] tracking-[0.28em] text-fg-dim xl:inline">
              <AUTHOR>
            </span>
          </div>
          ...
          {/*
            ...
            那一段固定项（品牌 115.5 + 分隔线 1 + 操作区 258 + 三个 24px 间隙）就占掉
            446.5px，1024 下 nav 只剩 527.5px，而 9 项按常规密度要 677px。
            调紧后（字号 13→12、左右内边距 12→6、项间距 4→0）内容降到 504px，正好放下。
            xl（1280）起富余够（nav 689 ≥ 677），所以**恢复常规密度** —— 主力宽度不受影响。

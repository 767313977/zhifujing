...
class Settings(BaseSettings):
    ...
    # 所以推送时刻也跟着提前）。
    #
    # 2026-09-23 再提到 **17:10**（用户要求：「这个时间龙虎榜已经出来了」）。
    # ⚠️ 这样离上面那次实测的「17:05 已有数据」只剩 **5 分钟**余量（17:30 时是 25 分钟）。
    # 哪天 17:10 采到的龙虎榜 / 机构席位为空，先怀疑这里 —— 挪回 17:30 再看一眼。
    # 单天的洞还能补：`POST /api/admin/backfill?start=&end=` 会带上龙虎榜
    # （三池那类漏了就补不回来，龙虎榜不受那个 15 天窗口限制）。
    #
    # ⚠️ **推送时刻就是这个时刻**：简报排在采集链中间（跑完日线 / 回补 / 形态扫描就发，
    # 见 `jobs/scheduler._run_daily`），所以真正到达时间比它晚一两分钟。想让推送正好落在
    # 17:10，这里得填 17:05。
    #
    # 两融/北向不受影响：那两项每次都回看 30 天，漏一天下次自然补上。
    scheduler_enabled: bool = True
    collect_hour: int = 17
    collect_minute: int = 10

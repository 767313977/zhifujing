...
# 「资金通道 / 国家队持股」这类**口径标签**：融资融券与沪深股通天然是全市场口径，
# 净额动辄几百亿 —— 实测 2026-09-22 融资融券 -424.3 亿、深股通 -280.3 亿，长期霸占
# 流出榜前三；国家大基金 / 证金持股这类持股标签同理，它们把榜单和「累计净流入」曲线
# 全带偏。**子串匹配**。
#
# 放在 models 而不是某个业务模块里，是因为**采集与查询两边都要用**：采集侧写库前就剔
# （`jobs/collect_flows.py`），查询侧并按进 `api/sector.py:BOARD_EXCLUDE` 那一串
# （业绩 / 身份 / 地域类）里一起当黑名单用 —— 后者是为了把「开始过滤之前已经落库的
# 那些」也挡住，否则接下来 30 天的累计曲线里还会一直挂着它们。
#
# 名字从 `FUND_FLOW_EXCLUDE` 改成这个（2026-09-23）：并进 BOARD_EXCLUDE 之后它不再是
# 「资金流榜专用」，叫原名会让人以为轮动矩阵那份名单与它无关。
#
# 与 `api/sector.py:BOARD_EXCLUDE` 里那串「业绩 / 身份 / 地域 / 事件」的分工：那些是
# **查询侧**才剔的（开始过滤之前落库的老数据也挡得住），这里是**采集写入前**就剔、
# 只收资金通道与国家队持股这四个。举牌 / 并购重组 / 股权转让 2026-09-23 起也在剔，
# 但归在 BOARD_EXCLUDE 那边（见它的说明）。
NON_THEME_EXCLUDE = (
    "融资融券",
    "股通",  # 沪股通 / 深股通 / 港股通
    "国家大基金",
...
class StockBasic(Base):
    ...
    **都只更新传入的那些列** —— 实测：只传 name 不会抹掉别的列，只传 industry 也不会
    ...
    - `updated_at` **不会被刷新**，它现在的含义是「这行的**首次插入**时间」，
...
class EtfShare(Base):
    """ETF 每日份额与行情快照。来源：**iFinD**（份额/收盘价/涨跌幅/成交额）
    + 同花顺（代码清单与「最新-交易日」），约 **20 次 iFinD 调用/日**。

    为什么要存**份额**而不是只看涨跌：份额变化就是真金白银的申赎 ——
    份额涨是净申购（钱进来）、跌是净赎回（钱出去），比「成交额」更能反映
    资金的真实方向（成交额只说明换手活跃）。

    换源记录：原来走 akshare 的东财 ETF 快照（`push2delay`），而那一系从本机与
    云端都连不上（实测 HTTP 000，同域名的 push2ex 正常）—— 该项在云端一直**静默
    失效**，页面上只表现为 ETF 面板空白。2026-09-22 换成 iFinD（见 §8.46）。
    """

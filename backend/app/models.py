...
class StockBasic(Base):
    """个股基础信息（代码 → 名称，另外还能放行业与市值）。

    原话是「仅覆盖自选股与选股结果，不做全市场落库」，2026-09-23 起多了一个写入方：
    **每天的 DDE 抓取**（`jobs/scan_dde.py`）顺手把全市场简称存进来。

    为什么要破例：那条路径本来就一次问全市场、响应里**本来就有「股票简称」**，
    只是没人取。而 `stock_daily` 只覆盖流动性池 + 池外涨停股，池外那一千来只在
    日线里查不到名字 —— 推送与 DDE 榜上就只剩代码（实测推出来每行都是
    「688084 688084」）。顺手存一份，比事后为「池外的票叫什么」再花一次调用划算。
    """
